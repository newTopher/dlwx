...
class WX_Remote_Opera{
    ...
    public function init($user,$password){  //初始化，登录微信平台
        ...
        */
        $this->user=$user;
        $this->cookieFile=Yii::getPathOfAlias('webroot').'/data/cookies/weixin/cookie_'.$this->user.'.txt';
        $this->loginFile=Yii::getPathOfAlias('webroot').'/data/cookies/weixin/login_'.$this->user.'.txt';
        $this->lastTimeFile=Yii::getPathOfAlias('webroot').'/data/cookies/weixin/last_'.$this->user.'.txt';
    ...
    public function test_login($user,$password){
        ...
        curl_setopt($ch,CURLOPT_POST,1);
        curl_setopt($ch,CURLOPT_POSTFIELDS,$post);
        curl_setopt($ch,CURLOPT_COOKIEJAR,Yii::getPathOfAlias('webroot').'/data/cookies/weixin/cookie.txt');
        $html=curl_exec($ch);
        preg_match('/[\?\&]token=(\d+)"/',$html,$t);
        if(count($t) == 0 ){
            return false;
    ...
    public function getheadimg($fromfakeid){
        ...
        curl_close($ch);
        $PNG_SAVE_DIR = Yii::getPathOfAlias('webroot').'/upload'.DIRECTORY_SEPARATOR.'weixin_headimg'.DIRECTORY_SEPARATOR;
    ...
    public function getqrcode($fromfakeid){
        ...
        $headimg=curl_exec($ch);
        curl_close($ch);
        $PNG_SAVE_DIR = Yii::getPathOfAlias('webroot').'/upload'.DIRECTORY_SEPARATOR.'weixin_qrcode'.DIRECTORY_SEPARATOR;
        $file = fopen($PNG_SAVE_DIR.$fromfakeid.".png","w");//打开文件准备写入
        fwrite($file,$headimg);//写入
        fclose($file);//关闭
    }
